...
class EvaluacionesSinComponentes extends React.Component {
    ...
    render() {
        ...
        let thead = <thead> <tr>
            <th>#</th> <th>Nombre</th> <th>Fecha</th> <th>Puntos</th>
        </tr></thead>;
        ...
        let tbody = evaluaciones.map((evaluacion, index) =>
            <tr key={index}>
                ...
                <td>{evaluacion.fecha ?
                    (new Date(1000 * evaluacion.fecha)).toLocaleString(): ""}</td>
    ...
    componentDidUpdate() {
        ...
        // Copiar el estado
        let { evaluacionesLengthKey, evaluacionesKeys } = JSON.parse(JSON.stringify(this.state));

        if (!evaluacionesLengthKey) { // Observar el método evaluacionesLength().
        ...
        if (changed) {
            this.setState({ evaluacionesLengthKey, evaluacionesKeys });
        }
    }
}
